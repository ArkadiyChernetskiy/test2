...
public:
    template<typename T>
    Matrix(const vector<vector<T>>& src) : buffer(vector<vector<Field>>(M, vector<Field>(N, Field(0)))) {
            ...
            for (int j = last_fixed_row + 1; j < static_cast<int>(M); ++j) {
                ...
                for (int k = last_fixed_row + 1; k < static_cast<int>(M); ++k) {
                    if (buffer[k][i] != Field(0))
                        add_row_to_row(last_fixed_row, k, (-buffer[k][i]) / buffer[last_fixed_row][i]);
                }
                break;
            }
        }
    ...
    Matrix inverted() const {
        ...
        int last_fixed_row = -1;
        for (int i = 0; i < static_cast<int>(N); ++i) {
            for (int j = last_fixed_row + 1; j < static_cast<int>(N); ++j) {
