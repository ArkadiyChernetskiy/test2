...
private:
    ...
    struct ControlBlockRegular : public BaseControlBlock {
        using BlockAlloc = typename std::allocator_traits<Alloc>::template rebind_alloc<ControlBlockRegular<Deleter, Alloc>>;
        ...
        ControlBlockRegular(T* obj, Deleter del, Alloc alloc) : del(del), alloc(alloc) {
            this->obj = obj;
        }
...
public:
    ...
    SharedPtr(T* ptr, Deleter del = Deleter(), Alloc alloc = Alloc()) {
        using ControlBlock = ControlBlockRegular<Deleter, Alloc>;
